 ...
 */
// seed.js — popula o CRM com dados de mockup realistas
// Uso: node seed.js   (apaga os dados atuais e recria)
const db = require('./db');

console.log('Limpando dados atuais...');
db.exec('DELETE FROM interacoes; DELETE FROM clientes;');
db.exec("DELETE FROM sqlite_sequence WHERE name IN ('clientes','interacoes');");

// Datas relativas a "hoje" = 2026-05-31 (alinhado com a tela Hoje)
const clientes = [
  {
  ...
  },
  {
    ...
    interacoes: [
      { texto: 'Quer automatizar a triagem de petições com IA. Marcou call de diagnóstico.', dias: '2026-05-29' },
    ],
  },
  {
    ...
    interacoes: [
      { texto: 'Após a palestra, pediram um projeto de capacitação de professores em IA generativa.', dias: '2026-05-22' },
    ],
  },
  {
    ...
    interacoes: [
      { texto: 'Lead chegou pelo Instagram perguntando sobre a mentoria de IA para pequenos negócios.', dias: '2026-05-30', ia: 1 },
    ],
  },
  {
    ...
    interacoes: [
      { texto: 'Fechamos! Projeto de automação de cotação de fretes com IA.', dias: '2026-05-27' },
    ],
  },
  {
    ...
    interacoes: [
      { texto: 'Assistiu um vídeo e quer um assistente de agendamento. Orçamento ainda indefinido.', dias: '2026-05-25' },
    ],
  },
  {
    ...
    interacoes: [
      { texto: 'Conheci no evento de IA. Querem integrar agentes ao produto SaaS deles.', dias: '2026-05-24' },
    ],
  },
  {
    ...
    proxima_acao: 'Primeiro contato', proxima_acao_data: '2026-06-04', created_by: 'humano',
    interacoes: [],
  },
  {
    ...
    interacoes: [
      { texto: 'Sem verba neste ciclo. Retomar contato no próximo semestre.', dias: '2026-05-18' },
    ],
  },
  {
...
const insertCliente = db.prepare(`
  ...
  VALUES
    (@nome, @empresa, @cargo, @telefone, @email, @tipo_cliente, @origem, @etapa, @resultado,
     @valor_estimado, @proposta_enviada, @status_pagamento, @proxima_acao, @proxima_acao_data, @created_by)
`);
const insertInteracao = db.prepare(
  'INSERT INTO interacoes (cliente_id, texto, gerado_por_ia, data) VALUES (?, ?, ?, ?)'
);

const tx = db.transaction(() => {
  for (const c of clientes) {
    const { interacoes, ...dados } = c;
    const info = insertCliente.run(dados);
    for (const i of (interacoes || [])) {
      insertInteracao.run(info.lastInsertRowid, i.texto, i.ia ? 1 : 0, i.dias);
    }
  }
});
tx();

const total = db.prepare('SELECT COUNT(*) n FROM clientes').get().n;
const totInter = db.prepare('SELECT COUNT(*) n FROM interacoes').get().n;
console.log(`OK! ${total} clientes e ${totInter} interações inseridos.`);
